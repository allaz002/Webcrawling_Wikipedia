...
"""
    ...
    def preprocess_text(self, text):
        ...
        # Stoppwort-Entfernung mit scikit-learn Liste
        from sklearn.feature_extraction.text import ENGLISH_STOP_WORDS
        german_stopwords = set(['der', 'die', 'das', 'und', 'oder', 'aber', 'in', 'von', 'zu', 
                               'mit', 'auf', 'für', 'ist', 'im', 'am', 'an', 'als', 'auch',
                               'es', 'ein', 'eine', 'einer', 'eines', 'dem', 'den', 'des'])
    ...
    def is_valid_url(self, url):
        ...
        for namespace in self.ignored_namespaces:
            if f'/{namespace}' in path or f'/wiki/{namespace}' in path:
                return False
    ...
    def print_progress_report(self):
        ...
        now = datetime.now()
        if (now - self.stats['last_report']).seconds >= self.report_interval:
...
Laufzeit: {runtime}
Links in Frontier: {len(self.frontier)}
Gecrawlte Seiten: {self.stats['total_crawled']}
...
"""
    ...
    def print_final_report(self):
        ...
        middle_start = len(sorted_pages) // 2 - 5
        middle_10 = sorted_pages[middle_start:middle_start+10] if len(sorted_pages) >= 20 else []
        bottom_10 = sorted_pages[-10:] if len(sorted_pages) >= 10 else []
